...
import { usePupilMutation, useUpdatePupil } from "@/api/Mutations";
import { usePupil, useShowAllPupilsQuery } from "@/api/Queries";
  ...
  useEffect(() => {
    if (pupil) {
      form.reset({
        ...
        datePassed: pupil.datePassed ? pupil.datePassed.split("T")[0] : "",
      });
    }
  }, [pupil, form]);
    ...
    updatePupil.mutate(data, {
      onSuccess: () => {
        toast.success("Pupil updated", {
          description: "Pupil details have been saved.",
        });
      },
      onError: (err: any) => {
        const backendMsg =
